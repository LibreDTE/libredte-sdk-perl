# This Makefile is for the LibreDTE::SDK extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/LibreDTE/SDK.pm]
#     AUTHOR => [q[Esteban De La Fuente Rubio, DeLaF <esteban[at]sasco.cl>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     LICENSE => q[lgpl]
#     NAME => q[LibreDTE::SDK]
#     PREREQ_PM => { JSON=>q[2.9], REST::Client=>q[88] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/LibreDTE/SDK.pm]
...
MAKE = make
FULLEXT = LibreDTE/SDK
BASEEXT = SDK
PARENT_NAME = LibreDTE
DLBASE = $(BASEEXT)
VERSION_FROM = lib/LibreDTE/SDK.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/LibreDTE/SDK.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIBDEP)$(DFSEP)Config.pm $(PERL_INCDEP)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/LibreDTE
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/LibreDTE
...
TO_INST_PM = lib/LibreDTE/SDK.pm

PM_TO_BLIB = lib/LibreDTE/SDK.pm \
	blib/lib/LibreDTE/SDK.pm
...
DISTNAME = LibreDTE-SDK
DISTVNAME = LibreDTE-SDK-0.02
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  lib/LibreDTE/SDK.pm $(INST_MAN3DIR)/LibreDTE::SDK.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''SDK para conectar una aplicación en Perl con LibreDTE'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Esteban De La Fuente Rubio, DeLaF <esteban[at]sasco.cl>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: LibreDTE-SDK' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '  REST::Client: '\''88'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'version: '\''0.02'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'x_serialization_backend: '\''CPAN::Meta::YAML version 0.018'\''' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "SDK para conectar una aplicación en Perl con LibreDTE",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Esteban De La Fuente Rubio, DeLaF <esteban[at]sasco.cl>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "LibreDTE-SDK",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "REST::Client" : "88"' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      }' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>SDK para conectar una aplicación en Perl con LibreDTE</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Esteban De La Fuente Rubio, DeLaF &lt;esteban[at]sasco.cl&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="JSON::" VERSION="2.9" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="REST::Client" VERSION="88" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/LibreDTE/SDK.pm blib/lib/LibreDTE/SDK.pm 
